...
from model import HopfEModel, HopfSemanticsEModel, HopfParaEModel, HopfParaE2Model, DensEModel
from dataloader import TrainDataset
from dataloader import BidirectionalOneShotIterator

import matplotlib.pyplot as plt

# usage: python vis_emb.py --data_path='/projects/RelationExtraction/KBC/HopfE-master/data/wn18/' --model="HopfParaE" -b=128 -lr=0.1 --max_steps=1000 -n=128 -g=9.0 -d=10 -adv -a=1.0 --init_checkpoint='/projects/RelationExtraction/KBC/HopfE-master/models/HopfParaE_wn18_besthyp_1L' --entity_embedding_has_mod --relation_embedding_has_mod --r1=1 --r2=6 --hist_analysis='inverse'
# python vis_emb.py --data_path='/home/anson/projects/RelationExtraction/KBC/HopfE-master/data/wn18rr/' --model="HopfParaE" -b=128 -lr=0.1 --max_steps=1000 -n=128 -g=9.0 -d=10 -adv -a=1.0 --init_checkpoint='/home/anson/projects/RelationExtraction/KBC/HopfE-master/models/HopfParaE_wn18rr_bestHyp_1L' --entity_embedding_has_mod --relation_embedding_has_mod --r1=1 --r2=0 --r3=1 --hist_analysis='composition'
# python vis_emb.py --data_path='/home/anson/projects/RelationExtraction/KBC/HopfE-master/data/wn18rr/' --model="HopfParaE" -b=128 -lr=0.1 --max_steps=1000 -n=128 -g=9.0 -d=10 -adv -a=1.0 --init_checkpoint='/home/anson/projects/RelationExtraction/KBC/HopfE-master/models/HopfParaE_wn18rr_bestHyp_1L' --entity_embedding_has_mod --relation_embedding_has_mod --r1=1 --r2=0 --r3=5 --hist_analysis='composition'
...
def parse_args(args=None):
    ...
    # wn18-> _has_part: 1, _part_of:6
    # wn18rr-> _derivationally_related_form:1, _hypernym:0, _derivationally_related_form:1
    # wn18rr-> _derivationally_related_form:1, _hypernym:0, _synset_domain_topic_of:5
    ...
    parser.add_argument('--hist_analysis', default='inverse', type=str, help='options: [inverse, composition]')
...
def save_hist(vector, save_path, bins='auto'):
    _ = plt.hist(vector, bins=bins)  # arguments are passed to np.histogram
    plt.title("")
    # plt.show()
    plt.savefig(save_path)
    plt.close()

def quat_mult(s_a, x_a, y_a, z_a, s_b, x_b, y_b, z_b):
...
def main(args):
    ...
    if args.hist_analysis=='inverse':
        r1 = torch.tensor(args.r1)
        r2 = torch.tensor(args.r2)

        w1 = kge_model.relation_w(r1).detach().numpy()
        x1 = kge_model.relation_x(r1).detach().numpy()
        y1 = kge_model.relation_y(r1).detach().numpy()
        z1 = kge_model.relation_z(r1).detach().numpy()

        w2 = kge_model.relation_w(r2).detach().numpy()
        x2 = kge_model.relation_x(r2).detach().numpy()
        y2 = kge_model.relation_y(r2).detach().numpy()
        z2 = kge_model.relation_z(r2).detach().numpy()

        norm1 = np.sqrt(w1**2 + x1**2 + y1**2 + z1**2)
        norm2 = np.sqrt(w2**2 + x2**2 + y2**2 + z2**2)
        w1 = np.divide(w1,norm1)
        w2 = np.divide(w2,norm2)

        pi = np.pi
        phi1 = 2*np.arccos(w1)
        phi2 = 2*np.arccos(w2)
        # q1 = np.linalg.norm([x1,y1,z1],axis=0)
        # q2 = np.linalg.norm([x2,y2,z2],axis=0)

        hist_phi = save_hist(phi1+phi2-2*pi, './tmp/hist_phi_inv.png')
        hist_q = save_hist(norm1*norm2, './tmp/hist_q_inv.png', bins=np.arange(0,2,0.1))
        hist_1 = save_hist(norm1, './tmp/hist_q1_inv.png', bins=np.arange(0,2,0.1))
        hist_2 = save_hist(norm2, './tmp/hist_q2_inv.png', bins=np.arange(0,2,0.1))

    elif args.hist_analysis=='composition':
        r1 = torch.tensor(args.r1)
        r2 = torch.tensor(args.r2)
        r3 = torch.tensor(args.r3)
        ...
        w21, x21, y21,z21 = quat_mult(w2, x2, y2, z2, w1, x1, y1, z1)
        norm21 = np.sqrt(w21**2 + x21**2 + y21**2 + z21**2)
        w21n = np.divide(w21,norm21)

        w3 = kge_model.relation_w(r3).detach().numpy()
        x3 = kge_model.relation_x(r3).detach().numpy()
        y3 = kge_model.relation_y(r3).detach().numpy()
        z3 = kge_model.relation_z(r3).detach().numpy()

        norm3 = np.sqrt(w3**2 + x3**2 + y3**2 + z3**2)
        w3n = np.divide(w3,norm3)

        phi21 = 2*np.arccos(w21n)
        phi3 = 2*np.arccos(w3n)

        hist_phi = save_hist(phi21-phi3, './tmp/hist_phi_comp.png')
