...
# python test_relations.py --data_path='/home/anson/projects/RelationExtraction/KBC/HopfE-master/data/wn18rr/' --model="HopfParaE" -b=128 -lr=0.1 --max_steps=1000 -n=128 -g=9.0 -d=10 -adv -a=1.0 --init_checkpoint='/home/anson/projects/RelationExtraction/KBC/HopfE-master/models/HopfParaE_wn18rr_bestHyp_1L' --entity_embedding_has_mod --relation_embedding_has_mod --cuda
...
def read_rel_wise_triples(file_path, entity2id, relation2id):
    '''
    Read triples and map them into ids.
    '''
    rel_wise_triples = {}
    with open(file_path) as fin:
        for line in fin:
            h, r, t = line.strip().split('\t')
            rel_wise_triples[r] = rel_wise_triples.get(r,[])
            rel_wise_triples[r].append((entity2id[h], relation2id[r], entity2id[t]))
    return rel_wise_triples
...
def log_metrics(mode, step, metrics, rel):
    '''
    Print the evaluation logs
    '''
    for metric in metrics:
        logging.info('%s %s %s at step %d: %f' % (rel, mode, metric, step, metrics[metric]))
...
def main(args):
    ...
    if args.test_file:
        ...
        _rel_wise_test_triples = read_rel_wise_triples(os.path.join(args.data_path, args.test_file), entity2id, relation2id)    
    else:
        _test_triples = read_triple(os.path.join(args.data_path, 'test.txt'), entity2id, relation2id)
        _rel_wise_test_triples = read_rel_wise_triples(os.path.join(args.data_path, 'test.txt'), entity2id, relation2id)
    ...
    for _rel, _rel_test_triples in _rel_wise_test_triples.items():
        if args.distributed:
            metrics = kge_model.module.test_step(kge_model, _rel_test_triples, _all_true_triples, args)
        else:
            metrics = kge_model.test_step(kge_model, _rel_test_triples, _all_true_triples, args)            
        log_metrics('Test', step, metrics, _rel)
